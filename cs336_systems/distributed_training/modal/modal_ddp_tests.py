"""Run ``tests/test_ddp.py`` on Modal (CPU + gloo; GPUs hidden).

Usage:

    modal run cs336_systems/distributed_training/modal/modal_ddp_tests.py
"""
...
app = modal.App("cs336-systems-ddp-tests")


@app.function(image=ddp_test_image, timeout=1800)
def run_pytest_remote() -> int:
    ...
    return int(pytest.main(["tests/test_ddp.py", "-v", "--tb=short", "-s"]))
