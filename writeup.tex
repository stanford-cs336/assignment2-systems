...
Figure~\ref{fig:memory-viz-per-block} shows the \texttt{pytorch.org/memory\_viz} active-memory timeline zoomed to the forward pass of the \emph{xl} model (batch 2, context 2048, FP32, from \texttt{xl\_ctx2048\_fbo\_fp32\_b2.pickle}), with Detail set to reveal the largest individual allocations. Clicking each colored band exposes the allocation size and Python/C++ stack trace.
...
The custom softmax in \texttt{nn\_utils.py} is implemented step-by-step rather than as a fused kernel, so PyTorch's autograd traces six separate $[B,H,T,T]$ allocations per attention layer:
\begin{enumerate}
  \item \textbf{Pink}: $\mathbf{QK}^\top$ --- output of \texttt{einsum} at \texttt{model.py:427}.
  \item \textbf{Brown}: $\mathbf{S}=\mathbf{QK}^\top/\sqrt{d}$ --- \texttt{div} at \texttt{model.py:427}.
  \item \textbf{Blue (bottom)}: masked scores --- \texttt{where} (causal mask) at \texttt{model.py:430}.
  \item \textbf{Light blue}: $\mathbf{S}-\max(\mathbf{S})$ --- \texttt{sub} at \texttt{nn\_utils.py:5} (numerical stability shift).
  \item \textbf{Green}: $\exp(\mathbf{S}-\max)$ --- \texttt{exp} at \texttt{nn\_utils.py:6}.
  \item \textbf{Purple}: $\mathbf{P}=\mathrm{softmax}(\mathbf{S})$ --- \texttt{div} at \texttt{nn\_utils.py:7}.
\end{enumerate}

\paragraph{SwiGLU/MLP tensors (each 160\,MiB, shape $[2,2048,10240]$ in FP32).}
Five further allocations appear from the feed-forward block:
\begin{enumerate}
  \item \textbf{Brown (top)}: $x_1=xW_1$ --- \texttt{einsum} through \texttt{Linear} at \texttt{model.py:39}.
  \item \textbf{Grey}: $\sigma(x_2)$ --- \texttt{sigmoid} at \texttt{model.py:532} inside \texttt{silu}.
  \item \textbf{Blue (top)}: $\mathrm{silu}(x_2)=x_2\cdot\sigma(x_2)$ --- \texttt{mul} at \texttt{model.py:532}.
  \item \textbf{Orange}: $x_2=xW_2$ --- \texttt{einsum} through \texttt{Linear} at \texttt{model.py:39}.
  \item \textbf{Red}: $z=x_1\cdot\mathrm{silu}(x_2)$ (SwiGLU gate output) --- \texttt{mul} at \texttt{model.py:399}.
\end{enumerate}

The dominant footprint is the six $1$\,GiB attention tensors ($6$\,GiB per block with batch 2, $12$\,GiB with batch 4). This reveals a key implementation detail: because the softmax is \emph{not} fused, autograd must save every intermediate --- raw scores, scaled scores, masked scores, stability-shifted scores, exponentials, and the final probabilities --- rather than just $\mathbf{S}$ and $\mathbf{P}$. FlashAttention and \texttt{torch.compile} eliminate this overhead by fusing the softmax into a single kernel that only emits $\mathbf{P}$ (or recomputes it on the fly), explaining their dramatic activation-memory savings.

During backward, saved tensors are freed while weight gradients are emitted. Weight gradients for one xl block total roughly $400$\,MiB: four attention projection matrices ($\mathbf{W}_Q,\mathbf{W}_K,\mathbf{W}_V,\mathbf{W}_O$) at $2560^2\times 4\,\text{B}\approx25$\,MiB each, plus the three MLP matrices ($W_1,W_2$ each $2560\times 10240\times 4\,\text{B}=100$\,MiB and $W_3$ at the same). The ${\gtrsim}6$\,GiB of dominant attention activations per block (batch 2) is more than $15\times$ the ${\sim}400$\,MiB of weight gradients produced, so peak memory during a full training step is activation-dominated rather than gradient-dominated. This is consistent with Table~\ref{tab:memory-peaks}: the full-step peak grows steeply with context length (reflecting $O(T^2)$ attention matrices) while the weight-gradient contribution is constant in $T$.
