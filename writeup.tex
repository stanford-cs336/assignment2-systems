...
\label{nsys_profile_context_fit_summary}
\end{table}
...
The Nsight forward-pass GPU kernel time agrees reasonably well with the Python benchmark. For example, the small model at context 4096 is 256.6\,ms in Nsight versus 294.9\,ms from Python, and the medium model at context 2048 is 252.9\,ms versus 274.0\,ms. The Python timing is slightly larger.
...
The dominant blocks are on the order of $\sim$2\,GiB per-layer, matching FP32 tensors shaped like pre-softmax scores or attention weights $[B, n_{\text{heads}}, T, T]$ (e.g.\ $4 \times 32 \times 2048^2 \times 4$\,bytes $\approx 2$\,GiB at batch $4$). Stack traces point to \texttt{scaled\_dot\_product\_attention} and the \texttt{einsum} to calculate $QK^\top$, then the \texttt{softmax} path that materializes the normalized weights.
...
\textbf{Removing this cost:} FlashAttention avoids materializing full $T\times T$ softmax tensors by tiling softmax with recomputed blocks on SRAM, with the same asymptotic compute but a much smaller HBM footprint.
